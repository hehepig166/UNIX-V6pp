...
public:
	...
	unsigned long	m_TextSize;			/* 代码段长度 */	// 包含只读数据段

	unsigned long	m_RDataStartAddress;	// 只读数据段起始地址
	unsigned long	m_RDataSize;			// 只读数据段长度

	unsigned long	m_DataStartAddress; /* 数据段起始地址 */
	unsigned long	m_DataSize;			/* 数据段长度 */	// 不包含只读数据段
